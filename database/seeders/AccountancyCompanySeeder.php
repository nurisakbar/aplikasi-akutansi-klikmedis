...
class AccountancyCompanySeeder extends Seeder
{
    public function run(): void
    {
        $this->command->info('Creating Accountancy Companies...');

        // Check if companies already exist
        $existingCount = AccountancyCompany::count();
        if ($existingCount > 0) {
            $this->command->info('Companies already exist (' . $existingCount . ' records). Skipping...');
            return;
        }

        $companies = [
            [
                'name' => 'KlikMedis Accounting Company',
                ...
                'website' => 'https://www.klikmedis.com',
            ],
            [
                'name' => 'Demo Healthcare Company',
                'address' => 'Jl. Sehat Sejahtera No. 456',
                'province' => 'Jawa Barat',
                'city' => 'Bandung',
                ...
                'website' => 'https://www.demohealthcare.com',
            ],
        ];

        foreach ($companies as $companyData) {
            $company = AccountancyCompany::create(array_merge($companyData, [
                'id' => (string) Str::uuid(),
                'created_at' => now(),
                'updated_at' => now(),
            ]));
        ...
        $this->command->info('Accountancy Companies created successfully!');
    }
    ...
    private function addAccount($data)
    {
        ...
        if (count($this->accounts) >= $this->batchSize) {
            DB::table('accountancy_chart_of_accounts')->insert($this->accounts);
            $this->accounts = [];
        }
    ...
    {
        $assetId = AccountancyChartOfAccount::where('company_id', $this->companyId)
            ->where('code', '1000')
            ->value('id');
    ...
    {
        $liabilityId = AccountancyChartOfAccount::where('company_id', $this->companyId)
            ->where('code', '3000')
            ->value('id');
    ...
    {
        $equityId = AccountancyChartOfAccount::where('company_id', $this->companyId)
            ->where('code', '4000')
            ->value('id');
    ...
    {
        $revenueId = AccountancyChartOfAccount::where('company_id', $this->companyId)
            ->where('code', '5000')
            ->value('id');
    ...
    {
        $expenseId = AccountancyChartOfAccount::where('company_id', $this->companyId)
            ->where('code', '6000')
            ->value('id');
