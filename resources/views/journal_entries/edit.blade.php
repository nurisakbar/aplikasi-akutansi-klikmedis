        ...
        <div class="card">
            ...
            <div class="card-body">
                ...
                <div class="form-group">
                    ...
                    <textarea name="description" id="description" class="form-control" rows="3">{{ old('description', $journalEntry->description) }}</textarea>
                ...
                <div class="form-group">
                    ...
                    @if($journalEntry->attachment)
                        <div class="mb-2">
                            <a href="{{ asset('storage/journal_attachments/' . $journalEntry->attachment) }}" target="_blank" class="btn btn-sm btn-info">
                                <i class="fas fa-download"></i> Download Lampiran Saat Ini
                            </a>
                        </div>
                    @endif
                    <input type="file" name="attachment" id="attachment" class="form-control" accept=".pdf,.jpg,.jpeg,.png">
                    <small class="form-text text-muted">Format yang diizinkan: PDF, JPG, JPEG, PNG. Maksimal 2MB. Kosongkan jika tidak ingin mengubah lampiran.</small>
                ...
                <table class="table table-bordered" id="lines-table">
                    ...
                    <tbody>
                        @php $oldLines = old('lines', $journalEntry->accountancyJournalEntryLines->toArray()); @endphp
                        @foreach($oldLines as $i => $line)
                            <tr>
                                <td>
                                    <select name="lines[{{ $i }}][chart_of_account_id]" class="form-control chart-account-select" required>
                                ...
                                <td>
                                    <input type="number" name="lines[{{ $i }}][debit]" class="form-control debit-input" step="0.01" value="{{ $line['debit'] }}" min="0" required>
                                    <div class="invalid-feedback"></div>
                                </td>
                                <td>
                                    <input type="number" name="lines[{{ $i }}][credit]" class="form-control credit-input" step="0.01" value="{{ $line['credit'] }}" min="0" required>
                                    <div class="invalid-feedback"></div>
                                </td>
                                <td>
                                    <input type="text" name="lines[{{ $i }}][description]" class="form-control" value="{{ $line['description'] ?? '' }}">
                                    <div class="invalid-feedback"></div>
                                </td>
                                <td>
...
$(document).ready(function() {
    ...
    $('#journal-entry-form').on('submit', function(e) {
        e.preventDefault();
        updateJournalEntry(this);
    });

    // Function untuk update journal entry
    function updateJournalEntry(formElement) {
        ...
        $.ajax({
            url: '{{ route('journal-entries.update', $journalEntry->id) }}',
            type: 'POST',
            data: formData,
            processData: false,
            contentType: false,
            success: function(response) {
                ...
                Swal.fire({
                    icon: 'success',
                    title: 'Berhasil!',
                    text: response.message || 'Jurnal berhasil diupdate.',
                    timer: 2000,
